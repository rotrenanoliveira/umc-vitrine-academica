...
interface RemoveStudentFromInstitutionRequest {
  memberId: string
  status: RemoveProfessorStatusParams
  actorId: string
}

type RemoveStudentFromInstitutionResponse = Either<
  InstitutionMemberNotFoundError | InstitutionMemberRoleMismatchError | UnauthorizedError,
  { member: InstitutionMember }
>

export class RemoveStudentFromInstitutionUseCase {
  ...
  async execute({
    ...
    status,
    actorId,
  }: RemoveStudentFromInstitutionRequest): Promise<RemoveStudentFromInstitutionResponse> {
    ...
    if (member.type !== InstitutionMemberType.STUDENT) {

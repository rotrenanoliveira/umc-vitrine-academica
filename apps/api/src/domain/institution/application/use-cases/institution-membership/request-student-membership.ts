...
interface RequestStudentMembershipRequest {
  institutionId: string
  userId: string
}

type RequestStudentMembershipResponse = Either<
...
export class RequestStudentMembershipUseCase {
  constructor(
  ...
  async execute({ institutionId, userId }: RequestStudentMembershipRequest): Promise<RequestStudentMembershipResponse> {
    ...
    const status =
      settings?.shouldVerify || settings?.shouldProof ? InstitutionMemberStatus.PENDING : InstitutionMemberStatus.ACTIVE
